...
x <- data.frame(x)
ggplot(x)+
  geom_histogram(aes(x = x), binwidth = 1, color = "white", fill = "gray")
```
...
#mean -- median -- 
#standard deviation from the mean - (mean_x + sd_x, mean_x - sd_x,) --
#absolute deviation from the median ( median_x + mad_x, median_x - mad_x)
# mad() -- median absolute deviation

ggplot(x, aes())+
  geom_histogram(aes(x),binwidth = 1, color = "white", fill = "gray") +
  geom_vline(xintercept = c(mean_x, 
                            median_x,
                            mean_x + sd_x,
                            mean_x - sd_x,
                            median_x + mad_x,
                            median_x - mad_x),
                 color = c("red", "red", "blue", "blue", "blue", "blue"), 
...
x<- rnorm(n = 1000000, mean = 10, sd = 5)
mean_x <- mean(x)
median_x <- median(x)
sd_x <- sd(x)
mad_x <- mad(x)
...
plot3 <- ggplot(data.frame(x = c(-5, 5)), aes(x = x)) +
...
gene_exprs <- as.matrix(gene_data)
...
#make df to get the gene name out of the way to make a numeric matrix

logcounts_df <- as.data.frame(logcounts_matrix)

# convert gene name to raw

logcounts_matrix <- column_to_rownames(logcounts_df, "gene")
...
cell_info_test <- unique(cell_info$Infection)

#x <- all(rownames(cell_info$Sample) == colnames(logcounts_matrix))

#pull names of 2 different groups

cells_infected <- cell_info %>%  filter(Infection == "STM-LT2") %>% pull(Sample)
  
cells_mock <- cell_info %>%  filter(Infection == "Mock") %>% pull(Sample)
...
# select cells for gene1 that are either moc or infected
expression_infected <- logcounts_matrix[1, cells_infected]
expression_uninfected <- logcounts_matrix[1, cells_mock]

row_1_test <- t.test(expression_infected, expression_uninfected)
...
expression_uninfected <- logcounts_matrix[index, cells_mock]

p_value <- t.test(expression_infected, expression_uninfected)
...
```{r}
gene_names <- row.names(logcounts_matrix)
...
#t_test <- as.data.frame(t_test_pvalues)

ggplot(t_test)+
  geom_histogram(aes(x = t_test_pvalues), binwidth = 0.02, color = "white", fill = "gray")
```
...
```{r}
result_bh <- p.adjust(t_test, method = "bonferroni")
