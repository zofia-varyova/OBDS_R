...
> Answer: Warning: 30 packages out-of-date; 0 packages too new
...
#quality check of the fastq file?
```
...
#NN not in genome, fixed = false, searches for the sequence with NN as any letter
```
...
#remove NAs
gtf_ACTB@elementMetadata@listData[["transcript_id"]] <- na.omit(gtf_ACTB@elementMetadata@listData[["transcript_id"]])

#uniqe transcripts
length(unique(gtf_ACTB@elementMetadata@listData[["transcript_id"]]))

#uniqe transcripts names
(unique(gtf_ACTB@elementMetadata@listData[["transcript_id"]]))
...
enzymes <- head(keys(org.Hs.eg.db, keytype = "ENZYME"))
...
```{r}
AnnotationDbi::select(org.Hs.eg.db, keys = human_symbols, columns = c("ENTREZID", "ENSEMBL", "GENENAME"), keytype = "SYMBOL")

select(org.Hs.eg.db, keys = human_symbols, columns = c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL")
```
...
assay_counts <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/counts.csv", row.names = "X")
...
demo_se <- SummarizedExperiment(assays = list(
                                          counts = assay_counts),
