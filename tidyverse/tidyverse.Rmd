...
```{r}
summary <- iris_raw %>% group_by(species) %>% 
...
test <- iris_raw %>% group_by(species) %>% 
...
```{r}
# Copy the code chunk above and extend with more pipes
 iris_raw %>% group_by(species) %>% summarise(across(where(is.numeric), mean)) %>% filter(sepal_length < 6) %>% arrange(desc(sepal_length))
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% group_by(species) %>% summarise(across(where(is.numeric), mean)) %>% filter(sepal_length < 6) %>% arrange(desc(sepal_length)) %>% pull(sepal_length, name = species)
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% mutate(sepal_area = sepal_length*sepal_width) %>% summarise(species, sepal_area)
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% mutate(sepal_area = sepal_length*sepal_width) %>% summarise(species, sepal_area) %>% arrange(desc(sepal_area)) %>% slice_head(n=5)
...
```{r}
iris_for_ggplot + facet_grid( . ~ species)
...
iris_long <- iris_raw %>% pivot_longer(cols = where(is.numeric),
...
iris_long2 <- iris_raw %>% mutate(ID = seq(1, nrow(iris_raw))) %>% unite("species", species:ID) %>% pivot_longer(cols = where(is.numeric),names_to = "Variables", values_to = "values")
...
```{r}
# Copy the code chunk above and refine to address the issue
iris_long2 %>% pivot_wider(names_from = Variables, values_from = values)
...
```{r}
library(readxl)
excel_sheets("/project/obds/zvaryova/base_r/projects/tidyverse/EH2011.xlsx")
exprs_assay <- read_excel("/project/obds/zvaryova/base_r/projects/tidyverse/EH2011.xlsx", "exprs")
...
```{r}
exprs_assay_long <- exprs_assay %>% pivot_longer(cols = where(is.numeric), names_to = "sample_ID")
...
ggplot(exprs_assay_long, aes(exprs, fill = gene)) +
...
sample_info <- read_excel("/project/obds/zvaryova/base_r/projects/tidyverse/EH2011.xlsx", "sample_info")
sample_info
```
...
sample_info <- sample_info %>% 
            mutate(across(
              c(sex, grade, T, N, M, vital_status, sample_type), list(as.factor),
              .names = "{.col}"))
...
```{r}
exprs_sampleinfo <- sample_info %>% left_join(exprs_assay_long, c("unique_patient_ID" = "sample"))
